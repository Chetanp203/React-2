import React from 'react'
import './MensTshirt.css'

const MensTshirt = () => {
    return (
        <div className='container'>
            <div className='c-1'>
                <h1>Men's winter wear</h1>
                <p>111853 Products</p>
            </div>

            <div className="c-2">
                <input type="text" value={'Sort by: Popularity'} />
            </div>

            <div className="inner-con">
                <div className="left">
                    <div className="l-block">
                        <div className="flex pad">
                            <h3>Filters</h3>
                            <p className="pink"><b>Clear All</b></p>
                        </div>
                        <div className='pad'>
                            <h3>Department</h3>
                            <button className='btn '>Mens Clothing</button>
                        </div>
                        <div className='pad'>
                            <h3>Category</h3>
                            <button className='btn '>Casual Wear</button>
                        </div>
                        <div className="pad">
                            <h3>Product Type</h3>
                            <div className="p-type">
                                <div className='flex '>
                                    <input type="radio" className='pr' />
                                    <p>T-shirts & Polos</p>
                                </div>
                                <p>40942</p>
                            </div>
                            <div className="p-type">
                                <div className='flex '>
                                    <input type="radio" className='pr' />
                                    <p>Shirts </p>
                                </div>
                                <p>31084</p>
                            </div>
                            <div className="p-type">
                                <div className='flex '>
                                    <input type="radio" className='pr' />
                                    <p>Sweatshirts</p>
                                </div>
                                <p>7435</p>
                            </div>
                            <div className="p-type">
                                <div className='flex '>
                                    <input type="radio" className='pr' />
                                    <p>Jackets</p>
                                </div>
                                <p>6520</p>
                            </div>
                            <div className="p-type">
                                <div className='flex '>
                                    <input type="radio" className='pr' />
                                    <p>Sweaters</p>
                                </div>
                                <p>3769</p>
                            </div>
                            <div className="p-type">
                                <div className='flex '>
                                    <input type="radio" className='pr' />
                                    <p>Jeans</p>
                                </div>
                                <p>10324</p>
                            </div>
                            <div className="p-type">
                                <div className='flex '>
                                    <input type="radio" className='pr' />
                                    <p>Trousers & Chinos</p>
                                </div>
                                <p>7291</p>
                            </div>
                            <div className="p-type">
                                <div className='flex '>
                                    <input type="radio" className='pr' />
                                    <p>Shorts</p>
                                </div>
                                <p>4377</p>
                            </div>
                            <div className="p-type">
                                <div className='flex '>
                                    <input type="radio" className='pr' />
                                    <p>Blazzers</p>
                                </div>
                                <p>328</p>
                            </div>
                            <div className="p-type">
                                <div className='flex '>
                                    <input type="radio" className='pr' />
                                    <p>Others</p>
                                </div>
                                <p>28</p>
                            </div>
                        </div>

                        <div className='pad flex'>
                            <h3>Brand</h3>
                            <i className="fa-solid fa-plus" style={{color:' #c2c2c2'}}></i>
                        </div>
                        <div className='pad flex'>
                            <h3>Size</h3>
                            <i className="fa-solid fa-plus" style={{color:' #c2c2c2'}}></i>
                        </div>
                        <div className='pad flex'>
                            <h3>Type</h3>
                            <i className="fa-solid fa-plus" style={{color:' #c2c2c2'}}></i>
                        </div>
                        <div className='pad flex'>
                            <h3>Colour</h3>
                            <i className="fa-solid fa-plus" style={{color:' #c2c2c2'}}></i>
                        </div>
                        <div className='pad flex'>
                            <h3>Price</h3>
                            <i className="fa-solid fa-plus" style={{color:' #c2c2c2'}}></i>
                        </div>
                        <div className='pad flex'>
                            <h3>Fit</h3>
                            <i className="fa-solid fa-plus" style={{color:' #c2c2c2'}}></i>
                        </div>
                        <div className='pad flex'>
                            <h3>Discount</h3>
                            <i className="fa-solid fa-plus" style={{color:' #c2c2c2'}}></i>
                        </div>
                        <div className='pad flex'>
                            <h3>Fabric Family</h3>
                            <i className="fa-solid fa-plus" style={{color:' #c2c2c2'}}></i>
                        </div>
                        <div className='pad flex'>
                            <h3>Sleeve</h3>
                            <i className="fa-solid fa-plus" style={{color:' #c2c2c2'}}></i>
                        </div>
                        <div className='pad flex'>
                            <h3>Collar</h3>
                            <i className="fa-solid fa-plus" style={{color:' #c2c2c2'}}></i>
                        </div>
                        <div className='pad flex'>
                            <h3>Pattern</h3>
                            <i className="fa-solid fa-plus" style={{color:' #c2c2c2'}}></i>
                        </div>
                        <div className='pad flex'>
                            <h3>Occasion</h3>
                            <i className="fa-solid fa-plus" style={{color:' #c2c2c2'}}></i>
                        </div>
                        <div className='pad flex'>
                            <h3>All Discount</h3>
                            <i className="fa-solid fa-plus" style={{color:' #c2c2c2'}}></i>
                        </div>
                        <div className='pad flex'>
                            <h3>Availability</h3>
                            <i className="fa-solid fa-plus" style={{color:' #c2c2c2'}}></i>
                        </div>



                    </div>

                </div>
                <div className="right">

                    <div className="product">
                        <img src="https://img.tatacliq.com/images/i9/437Wx649H/MP000000016373929_437Wx649H_202302042021191.jpeg" />
                        <h3>U.S. Polo Assn</h3>
                        <p>U.S.Polo.Assn. Ivory Cotton</p>
                        <p>Regular Fit Polo T-Shirt</p>
                        <h3>₹1899</h3>
                        <button className='rating'>
                            2.8
                            <i className="fa-solid fa-star " style={{color: '#0dc92c'}}></i>
                        </button>
                        <h5 className='orange'>Limited stock!</h5>
                    </div>
                    
                    <div className="product">
                        <img src="https://img.tatacliq.com/images/i8/437Wx649H/MP000000014765428_437Wx649H_202210061555121.jpeg" />
                        <h3>UCB</h3>
                        <p>United Colours of Benetton</p>
                        <p>Maroon Polo T-Shirt</p>
                        <h3>₹1034</h3>
                        <button className='rating'>
                            3
                            <i className="fa-solid fa-star " style={{color: '#0dc92c'}}></i>
                        </button>
                        <h5 className='orange'>Limited stock!</h5>
                    </div>
                    
                    <div className="product">
                        <img src="https://img.tatacliq.com/images/i8/437Wx649H/MP000000015247442_437Wx649H_202211110452311.jpeg" />
                        <h3>Jack & Jones</h3>
                        <p>Jack & Jones Navy Blue</p>
                        <p>Cotton Slim Fit Polo T-Shirt</p>
                        <h3>₹764</h3>
                        <button className='rating'>
                            4.5
                            <i className="fa-solid fa-star " style={{color: '#0dc92c'}}></i>
                        </button>
                        <h5 className='orange'>Limited stock!</h5>
                    </div>
                    
                    <div className="product">
                        <img src="https://img.tatacliq.com/images/i8/437Wx649H/MP000000014428153_437Wx649H_202209112250181.jpeg" />
                        <h3>U.S.Polo Assn.</h3>
                        <p>U.S.Polo.Assn. Dark Green</p>
                        <p>Cotton Regular Fit Printed</p>
                        <h3>₹1599</h3>
                        <button className='rating'>
                            3
                            <i className="fa-solid fa-star " style={{color: '#0dc92c'}}></i>
                        </button>
                        <h5 className='orange'>Limited stock!</h5>
                    </div>
                    
                    <div className="product">
                        <img src="https://img.tatacliq.com/images/i9/437Wx649H/MP000000016362070_437Wx649H_202302040017141.jpeg" />
                        <h3>Red Tape </h3>
                        <p>Red Tape White Regular Fit</p>
                        <p>Graphic Print Crew Fit</p>
                        <h3>₹417</h3>
                        <button className='rating'>
                            4.5
                            <i className="fa-solid fa-star " style={{color: '#0dc92c'}}></i>
                        </button>
                        <h5 className='orange'>Limited stock!</h5>
                    </div>
                    
                    <div className="product">
                        <img src="https://img.tatacliq.com/images/i11/437Wx649H/MP000000017261620_437Wx649H_202305161201101.jpeg" />
                        <h3>Red Tape </h3>
                        <p>Red Tape Black Cotton</p>
                        <p>Graphic Print Crew Fit</p>
                        <h3>₹373</h3>
                        <button className='rating'>
                            4.1
                            <i className="fa-solid fa-star " style={{color: '#0dc92c'}}></i>
                        </button>
                        <p className='flex'>Price dropped by <p className='pink'><b>Rs.136</b></p></p>
                    </div>
                    
                    <div className="product">
                        <img src="https://img.tatacliq.com/images/i10/437Wx649H/MP000000017210419_437Wx649H_202304141358201.jpeg" />
                        <h3>Jack & Jones</h3>
                        <p>Jack & Jones Dark Grey Low</p>
                        <p>Rise Jeans</p>
                        <h3>₹1899</h3>
                        <button className='rating'>
                            2.8
                            <i className="fa-solid fa-star " style={{color: '#0dc92c'}}></i>
                        </button>
                        <h5 className='orange'>Limited stock!</h5>
                    </div>
                    
                    <div className="product">
                        <img src="https://img.tatacliq.com/images/i9/437Wx649H/MP000000016236811_437Wx649H_202301261320511.jpeg" />
                        <h3>Jack & Jones</h3>
                        <p>Jack & Jones Dark Grey Low</p>
                        <p>Rise Jeans</p>
                        <h3>₹1899</h3>
                        <button className='rating'>
                            2.8
                            <i className="fa-solid fa-star " style={{color: '#0dc92c'}}></i>
                        </button>
                        <h5 className='orange'>Limited stock!</h5>
                    </div>
                    
                    <div className="product">
                        <img src="https://img.tatacliq.com/images/i9/437Wx649H/MP000000016373929_437Wx649H_202302042021191.jpeg" />
                        <h3>U.S. Polo Assn</h3>
                        <p>U.S.Polo.Assn. Ivory Cotton</p>
                        <p>Regular Fit Polo T-Shirt</p>
                        <h3>₹1899</h3>
                        <button className='rating'>
                            2.8
                            <i className="fa-solid fa-star " style={{color: '#0dc92c'}}></i>
                        </button>
                        <h5 className='orange'>Limited stock!</h5>
                    </div>
                    
                    <div className="product">
                        <img src="https://img.tatacliq.com/images/i8/437Wx649H/MP000000014765428_437Wx649H_202210061555121.jpeg" />
                        <h3>UCB</h3>
                        <p>United Colours of Benetton</p>
                        <p>Maroon Polo T-Shirt</p>
                        <h3>₹1034</h3>
                        <button className='rating'>
                            3
                            <i className="fa-solid fa-star " style={{color: '#0dc92c'}}></i>
                        </button>
                        <h5 className='orange'>Limited stock!</h5>
                    </div>
                    
                    <div className="product">
                        <img src="https://img.tatacliq.com/images/i8/437Wx649H/MP000000015247442_437Wx649H_202211110452311.jpeg" />
                        <h3>Jack & Jones</h3>
                        <p>Jack & Jones Navy Blue</p>
                        <p>Cotton Slim Fit Polo T-Shirt</p>
                        <h3>₹764</h3>
                        <button className='rating'>
                            4.5
                            <i className="fa-solid fa-star " style={{color: '#0dc92c'}}></i>
                        </button>
                        <h5 className='orange'>Limited stock!</h5>
                    </div>
                    
                    <div className="product">
                        <img src="https://img.tatacliq.com/images/i8/437Wx649H/MP000000014428153_437Wx649H_202209112250181.jpeg" />
                        <h3>U.S.Polo Assn.</h3>
                        <p>U.S.Polo.Assn. Dark Green</p>
                        <p>Cotton Regular Fit Printed</p>
                        <h3>₹1599</h3>
                        <button className='rating'>
                            3
                            <i className="fa-solid fa-star " style={{color: '#0dc92c'}}></i>
                        </button>
                        <h5 className='orange'>Limited stock!</h5>
                    </div>
                    
                    <div className="product">
                        <img src="https://img.tatacliq.com/images/i9/437Wx649H/MP000000016362070_437Wx649H_202302040017141.jpeg" />
                        <h3>Red Tape </h3>
                        <p>Red Tape White Regular Fit</p>
                        <p>Graphic Print Crew Fit</p>
                        <h3>₹417</h3>
                        <button className='rating'>
                            4.5
                            <i className="fa-solid fa-star " style={{color: '#0dc92c'}}></i>
                        </button>
                        <h5 className='orange'>Limited stock!</h5>
                    </div>
                    
                    <div className="product">
                        <img src="https://img.tatacliq.com/images/i11/437Wx649H/MP000000017261620_437Wx649H_202305161201101.jpeg" />
                        <h3>Red Tape </h3>
                        <p>Red Tape Black Cotton</p>
                        <p>Graphic Print Crew Fit</p>
                        <h3>₹373</h3>
                        <button className='rating'>
                            4.1
                            <i className="fa-solid fa-star " style={{color: '#0dc92c'}}></i>
                        </button>
                        <p className='flex'>Price dropped by <p className='pink'><b>Rs.136</b></p></p>
                    </div>
                    
                    <div className="product">
                        <img src="https://img.tatacliq.com/images/i10/437Wx649H/MP000000017210419_437Wx649H_202304141358201.jpeg" />
                        <h3>Jack & Jones</h3>
                        <p>Jack & Jones Dark Grey Low</p>
                        <p>Rise Jeans</p>
                        <h3>₹1899</h3>
                        <button className='rating'>
                            2.8
                            <i className="fa-solid fa-star " style={{color: '#0dc92c'}}></i>
                        </button>
                        <h5 className='orange'>Limited stock!</h5>
                    </div>
                    
                    <div className="product">
                        <img src="https://img.tatacliq.com/images/i9/437Wx649H/MP000000016236811_437Wx649H_202301261320511.jpeg" />
                        <h3>Jack & Jones</h3>
                        <p>Jack & Jones Dark Grey Low</p>
                        <p>Rise Jeans</p>
                        <h3>₹1899</h3>
                        <button className='rating'>
                            2.8
                            <i className="fa-solid fa-star " style={{color: '#0dc92c'}}></i>
                        </button>
                        <h5 className='orange'>Limited stock!</h5>
                    </div>
                    
                    




                </div>
            </div>
        </div>
    )
}

export default MensTshirt